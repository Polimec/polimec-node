...
impl From<LockError> for XcmError {
	fn from(e: LockError) -> XcmError {
		...
		match e {
			...
			WouldClobber | NotLocked | NotEnoughLocked | Unimplemented | NotTrusted | BadOwner | UnknownAsset |
			AssetNotOwned | NoResources | UnexpectedState | InUse => XcmError::LockError,
		}
	}
}
...
impl AssetLock for () {
	...
	fn prepare_reduce_unlockable(_: Location, _: Asset, _: Location) -> Result<Self::ReduceTicket, LockError> {
		Err(LockError::NotApplicable)
	}
}
