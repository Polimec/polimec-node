...
impl AssetsInHolding {
	...
	pub fn fungible_assets_iter(&self) -> impl Iterator<Item = Asset> + '_ {
		self.fungible.iter().map(|(id, &amount)| Asset { fun: Fungible(amount), id: id.clone() })
	}

	/// A borrowing iterator over the non-fungible assets.
	pub fn non_fungible_assets_iter(&self) -> impl Iterator<Item = Asset> + '_ {
		self.non_fungible.iter().map(|(id, instance)| Asset { fun: NonFungible(*instance), id: id.clone() })
	}
	...
	pub fn into_assets_iter(self) -> impl Iterator<Item = Asset> {
		self.fungible
			...
			.chain(self.non_fungible.into_iter().map(|(id, instance)| Asset { fun: NonFungible(instance), id }))
	}
	...
	pub fn subsume(&mut self, asset: Asset) {
		match asset.fun {
			Fungible(amount) => {
				self.fungible.entry(asset.id).and_modify(|e| *e = e.saturating_add(amount)).or_insert(amount);
			},
	...
	pub fn reanchor(&mut self, target: &Location, context: &InteriorLocation, mut maybe_failed_bin: Option<&mut Self>) {
		let mut fungible = Default::default();
		mem::swap(&mut self.fungible, &mut fungible);
		self.fungible = fungible
			.into_iter()
	...
	pub fn contains_asset(&self, asset: &Asset) -> bool {
		match asset {
			Asset { fun: Fungible(amount), id } => self.fungible.get(id).map_or(false, |a| a >= amount),
			Asset { fun: NonFungible(instance), id } => self.non_fungible.contains(&(id.clone(), *instance)),
		}
	}
	...
	fn general_take(&mut self, mask: AssetFilter, saturate: bool) -> Result<AssetsInHolding, TakeError> {
