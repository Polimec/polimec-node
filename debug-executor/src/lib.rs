...
pub mod traits;
use traits::{
	validate_export, AssetExchange, AssetLock, CallDispatcher, ClaimAssets, ConvertOrigin, DropAssets, Enact,
	ExportXcm, FeeManager, FeeReason, HandleHrmpChannelAccepted, HandleHrmpChannelClosing,
	HandleHrmpNewChannelOpenRequest, OnResponse, ProcessTransaction, Properties, ShouldExecute, TransactAsset,
...
impl<Config: config::Config> ExecuteXcm<Config::RuntimeCall> for XcmExecutor<Config> {
	...
	fn execute(
		...
		if let Err(e) = Config::Barrier::should_execute(&origin, message.inner_mut(), xcm_weight, &mut properties) {
			log::trace!(
				target: "xcm::execute",
...
impl From<ExecutorError> for frame_benchmarking::BenchmarkError {
	fn from(error: ExecutorError) -> Self {
		log::error!("XCM ERROR >> Index: {:?}, Error: {:?}, Weight: {:?}", error.index, error.xcm_error, error.weight);
...
impl<Config: config::Config> XcmExecutor<Config> {
	...
	pub fn post_process(mut self, xcm_weight: Weight) -> Outcome {
		...
		if !self.holding.is_empty() {
			...
			let trap_weight = Config::AssetTrap::drop_assets(effective_origin, self.holding, &self.context);
	...
	fn send(&mut self, dest: Location, msg: Xcm<()>, reason: FeeReason) -> Result<XcmHash, XcmError> {
		log::trace!(
	...
	fn refund_surplus(&mut self) -> Result<(), XcmError> {
		...
		if current_surplus.any_gt(Weight::zero()) {
			if let Some(w) = self.trader.refund_weight(current_surplus, &self.context) {
				if !self.holding.contains_asset(&(w.id.clone(), 1).into()) && self.ensure_can_subsume_assets(1).is_err()
				{
					let _ = self
						.trader
						.buy_weight(current_surplus, w.into(), &self.context)
						.defensive_proof("refund_weight returned an asset capable of buying weight; qed");
					log::error!(
						target: "xcm::refund_surplus",
	...
	fn to_querier(local_querier: Option<Location>, destination: &Location) -> Result<Option<Location>, XcmError> {
	...
	fn try_reanchor<T: Reanchorable>(
		...
		let reanchored = reanchorable.reanchored(&destination, &reanchor_context).map_err(|error| {
	...
	fn process_instruction(&mut self, instr: Instruction<Config::RuntimeCall>) -> Result<(), XcmError> {
		...
		match instr {
			...
			Transact { origin_kind, require_weight_at_most, mut call } => {
				...
				let dispatch_origin =
					Config::OriginConverter::convert_origin(origin.clone(), origin_kind).map_err(|_| {
						log::trace!(
							target: "xcm::process_instruction::transact",
			...
			InitiateReserveWithdraw { assets, reserve, xcm } => {
				...
				let result = Config::TransactionalProcessor::process(|| {
					...
					// back into Holding.
					let assets =
						Self::reanchored(self.holding.saturating_take(assets), &reserve, Some(&mut self.holding));
			...
			ReportHolding { response_info, assets } => {
				...
				// from Holding.
				let assets = Self::reanchored(self.holding.min(&assets), &response_info.destination, None);
				self.respond(self.cloned_origin(), Response::Assets(assets), response_info, FeeReason::Report)?;
				Ok(())
			},
			BuyExecution { fees, weight_limit } => {
				...
				let max_fee = self.holding.try_take(fees.into()).map_err(|_| XcmError::NotHoldingFees)?;
				log::trace!(
					target: "xcm::execute",
					"max_fee: {max_fee:?}",
				);
			...
			ExpectAsset(assets) => self.holding.ensure_contains(&assets).map_err(|_| XcmError::ExpectationFalse),
			ExpectOrigin(origin) => {
			...
			ExportMessage { network, destination, xcm } => {
				...
				let universal_source =
					Config::UniversalLocation::get().within_global(origin).map_err(|()| XcmError::Unanchored)?;
			...
			LockAsset { asset, unlocker } => {
				...
				let result = Config::TransactionalProcessor::process(|| {
					...
					let owner = origin.reanchored(&unlocker, &context).map_err(|_| XcmError::ReanchorFailed)?;
			...
			RequestUnlock { asset, locker } => {
				...
				let msg = Xcm::<()>(vec![UnlockAsset { asset: remote_asset, target: remote_target }]);
			...
			HrmpChannelClosing { initiator, sender, recipient } => Config::TransactionalProcessor::process(|| {
