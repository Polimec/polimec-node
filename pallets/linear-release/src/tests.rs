...
fn vested_transfer_correctly_fails() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		let user2_free_balance = Balances::free_balance(2);
		let user4_free_balance = Balances::free_balance(4);
		assert_eq!(user2_free_balance, ED);
...
fn vesting_info_validate_works() {
	...
	assert!(VestingInfo::new(min_transfer - 1, 1u64, 10u64).is_valid());

	// `locked` cannot be 0.
	assert!(!VestingInfo::new(0, 1u64, 10u64).is_valid());

	// `per_block` cannot be 0.
	assert!(!VestingInfo::new(min_transfer + 1, 0u64, 10u64).is_valid());

	// With valid inputs it does not error.
	assert!(VestingInfo::new(min_transfer, 1u64, 10u64).is_valid());
}
...
fn vested_transfer_less_than_existential_deposit_fails() {
	ExtBuilder::default().existential_deposit(4 * ED).build().execute_with(|| {
		...
		let sched = VestingInfo::new(<Test as Config>::MinVestedTransfer::get(), 1u64, 10u64);
