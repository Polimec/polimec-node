...
mod claim {
    ...
    fn user_can_claim_for_free() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let jwt = get_mock_jwt(1, InvestorType::Retail);
    ...
    // TODO: Enable once new did pr is merged
    // #[test]
    ...
    //         for i in 1..=x {
    //             let jwt = get_mock_jwt(i, InvestorType::Retail);
    ...
    //         }
    //         // Faucet is empty.
    //         assert_noop!(Faucet::claim(RuntimeOrigin::signed(x + 1), get_mock_jwt(x + 1, InvestorType::Retail)), Error::<Test>::FaucetDepleted);
    //     });
    // }
}