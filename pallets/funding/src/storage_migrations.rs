...
pub mod v6 {
	...
	use sp_runtime::{traits::Header, WeakBoundedVec};
	...
	impl<T: Config> UncheckedOnRuntimeUpgrade for UncheckedMigrationToV6<T> {
		fn on_runtime_upgrade() -> frame_support::weights::Weight {
			...
			let mut old_migration_keys = v5_storage_items::UserMigrations::<T>::iter();
			let len = old_migration_keys.collect_vec().len();
			log::info!("Found {} old migrations", len);
