...
//! 			let project_id: <T as pallet_funding::Config>::ProjectIdentifier =
//! 				project_id.into();
//! 			// Check project is in the community round
//! 			let project_details = pallet_funding::Pallet::<T>::project_details(project_id)
//! 				.ok_or(Error::<T>::ProjectNotFound)?;
//! 			ensure!(
//! 				project_details.status == pallet_funding::ProjectStatus::CommunityRound,
//! 				"Project is not in the community round"
//! 			);
//!
//! 			// Calculate how much funding was done already
//! 			let project_contributions: <T as pallet_funding::Config>::Balance =
//! 				pallet_funding::Contributions::<T>::iter_prefix_values((project_id,)).fold(
//! 					0u64.into(),
...
//! 				1u8.try_into().map_err(|_| Error::<T>::ProjectNotFound)?;
//! 			// Buy tokens with the default multiplier
//! 			pallet_funding::Pallet::<T>::do_contribute(
//! 				&retail_user,
//! 				project_id,
//! 				amount,
//! 				multiplier,
