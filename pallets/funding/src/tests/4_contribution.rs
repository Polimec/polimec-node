...
mod round_flow {
	...
	mod success {
		...
		fn contribution_round_ends_on_all_ct_sold_exact() {
			...
			inst.do_free_funding_asset_assertions(vec![UserToForeignAssets::<TestRuntime>::new(
				BOB,
				0_u128,
		...
		fn different_decimals_ct_works_as_expected() {
			...
			let mut decimal_test = |decimals: u8| {
				...
				inst.mint_funding_asset_to(vec![UserToForeignAssets::new(
					BUYER_1,
	...
	mod failure {
		...
		fn contribution_errors_if_user_limit_is_reached() {
			...
			assert_eq!(
				...
				inst.sum_foreign_asset_mappings(vec![foreign_funding.clone()])[0].1
			);
		}
