...
mod benchmarks {
	...
	fn bid(
		...
		let mut usdt_for_filler_bidder =
			vec![UserToForeignAssets::<T>::new(new_bidder.clone(), Zero::zero(), AcceptedFundingAsset::USDT.id())];
		if y > 0 {
	...
	fn contribute(
		...
		let total_usdt_locked = inst.sum_foreign_asset_mappings(vec![prev_total_usdt_locked, usdt.clone()])[0].1;
