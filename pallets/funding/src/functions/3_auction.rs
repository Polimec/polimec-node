...
impl<T: Config> Pallet<T> {
	...
	pub fn do_process_next_oversubscribed_bid(project_id: ProjectId) -> DispatchResult {
		...
		if let Some((price, index)) = maybe_current_cutoff {
			let bid = Bids::<T>::get((project_id, index)).ok_or(Error::<T>::ImpossibleState)?;
			...
			let first_bucket_bounds =
				BidBucketBounds::<T>::get(project_id, first_price).ok_or(Error::<T>::ImpossibleState)?;
			current_cutoff = (first_price, first_bucket_bounds.1);
		}

		let (_price, index) = current_cutoff;

		let mut bid = Bids::<T>::get((project_id, index)).ok_or(Error::<T>::ImpossibleState)?;
	...
	pub fn get_next_cutoff(
		...
		let bounds = BidBucketBounds::<T>::get(project_id, current_price).ok_or(Error::<T>::ImpossibleState)?;
		if current_index == bounds.0 {
			let new_price = current_price.saturating_add(delta_price);
			let new_bounds = BidBucketBounds::<T>::get(project_id, new_price).ok_or(Error::<T>::ImpossibleState)?;
			Ok((new_price, new_bounds.1))
		} else {
