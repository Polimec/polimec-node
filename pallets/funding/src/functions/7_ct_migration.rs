...
impl<T: Config> Pallet<T> {
	...
	pub fn do_start_offchain_migration(project_id: ProjectId, caller: AccountIdOf<T>) -> DispatchResult {
		...
		// * Update storage *
		let offchain_migration_info = OffchainMigrationInfo { remaining_participants: UserMigrations::<T>::count() };

		project_details.migration_type = Some(MigrationType::Offchain(offchain_migration_info));
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_start_pallet_migration(
		...
		let parachain_receiver_pallet_info = PalletMigrationInfo {
			...
			remaining_participants: UserMigrations::<T>::count(),
		};
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_mark_project_ct_migration_as_finished(project_id: ProjectId) -> DispatchResult {
		...
		let remaining_participants = match project_details.migration_type {
			Some(MigrationType::Pallet(ref info)) => info.remaining_participants,
			Some(MigrationType::Offchain(ref info)) => info.remaining_participants,
			None => return Err(Error::<T>::NotAllowed.into()),
		};
		ensure!(remaining_participants == 0, Error::<T>::MigrationsStillPending);
