	...
	> Instantiator<T, AllPalletsWithoutSystem, RuntimeEvent>
{
	...
	pub fn get_all_free_funding_asset_balances(&mut self, asset_id: AssetIdOf<T>) -> Vec<UserToForeignAssets<T>> {
		let user_keys = self.execute(|| frame_system::Account::<T>::iter_keys().collect());
	...
	pub fn mint_funding_asset_to(&mut self, mapping: Vec<UserToForeignAssets<T>>) {
		self.execute(|| {
	...
	pub fn do_free_funding_asset_assertions(&mut self, correct_funds: Vec<UserToForeignAssets<T>>) {
		for UserToForeignAssets { account, asset_amount: expected_amount, asset_id } in correct_funds {
			self.execute(|| {
