...
use frame_support::{assert_noop, assert_ok, traits::{
	fungible::{Inspect, InspectHold, Mutate as FungibleMutate},
	fungibles::{metadata::Mutate as MetadataMutate, Create, Inspect as FungiblesInspect, Mutate},
	Get,
}};
...
fn locked_outcome() {
	new_test_ext().execute_with(|| {
		...
		let expected_fee = 5 * FEE_ASSET_UNIT;

		// User decides to pay the fee with asset 1337
		let fee_asset = 1337u32;
		...
		assert_eq!(<Assets as FungiblesInspect<u64>>::balance(fee_asset, &sub_account_0), expected_fee);
		...
		assert_noop!(
			ProxyBonding::transfer_bonds_back_to_treasury(RuntimeOrigin::signed(1), derivation_path, hold_reason.clone()),
			Error::<TestRuntime>::TooEarlyToUnlock
		);
		assert_ok!(ProxyBonding::transfer_fees_to_recipient(RuntimeOrigin::signed(1), derivation_path, hold_reason.clone(), fee_asset));
		assert_eq!(<Assets as FungiblesInspect<u64>>::balance(fee_asset, &fee_recipient), expected_fee);

		System::set_block_number(10);

		assert_ok!(ProxyBonding::transfer_bonds_back_to_treasury(RuntimeOrigin::signed(1), derivation_path, hold_reason.clone()));
...
fn refunded_outcome() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(<Balances as Inspect<u64>>::balance(&treasury), ed + bond_amount);
		assert_noop!(ProxyBonding::transfer_fees_to_recipient(RuntimeOrigin::signed(1), derivation_path, hold_reason.clone(), fee_asset), Error::<TestRuntime>::FeeToRecipientDisallowed);

		assert_ok!(
			ProxyBonding::refund_fee(derivation_path, hold_reason.clone(), user, bond_amount, fee_asset)
		);
