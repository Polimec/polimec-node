...
impl FetchPrice for KrakenFetcher {
	...
	fn get_url(name: AssetName) -> &'static str {
		match name {
			...
			AssetName::PLMC => "",
			AssetName::ETH => "https://api.kraken.com/0/public/OHLC?pair=XETHZUSD&interval=1",
		}
	}
}
...
impl FetchPrice for BitFinexFetcher {
	...
	fn get_url(name: AssetName) -> &'static str {
		match name {
			...
			AssetName::ETH => "https://api-pub.bitfinex.com/v2/candles/trade%3A1m%3AtETHUSD/hist?limit=15",
			_ => "",
		}
	}
}
...
impl FetchPrice for BitStampFetcher {
	...
	fn get_url(name: AssetName) -> &'static str {
		match name {
			...
			AssetName::PLMC => "",
			AssetName::ETH => "https://www.bitstamp.net/api/v2/ohlc/ethusd/?step=60&limit=15",
		}
	}
}
...
impl FetchPrice for CoinbaseFetcher {
	...
	fn get_url(name: AssetName) -> &'static str {
		match name {
			...
			AssetName::ETH => "https://api.exchange.coinbase.com/products/ETH-USD/candles?granularity=60",
			_ => "",
		}
	}
}
