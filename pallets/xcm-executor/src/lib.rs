...
use xcm_executor::traits::{
	...
	ExportXcm, FeeManager, FeeReason, OnResponse, ShouldExecute, TransactAsset, VersionChangeNotifier, WeightBounds,
	WeightTrader,
};
...
impl<Config: config::Config> ExecuteXcm<Config::RuntimeCall> for XcmExecutor<Config> {
	...
	fn execute(
		...
		if let Err(e) = Config::Barrier::should_execute(&origin, message.inner_mut(), xcm_weight, &mut weight_credit) {
			log::trace!(
				target: "xcm::execute_xcm_in_credit",
	...
	fn process_instruction(&mut self, instr: Instruction<Config::RuntimeCall>) -> Result<(), XcmError> {
		...
		match instr {
			...
			Transact { origin_kind, require_weight_at_most, mut call } => {
				...
				let dispatch_origin =
					Config::OriginConverter::convert_origin(origin, origin_kind).map_err(|_| XcmError::BadOrigin)?;
			...
			msg @ HrmpNewChannelOpenRequest { .. } =>
				Config::HrmpHandler::handle_channel_open_request(msg.into()),
			msg @ HrmpChannelAccepted { .. } =>
				Config::HrmpHandler::handle_channel_accepted(msg.into()),
