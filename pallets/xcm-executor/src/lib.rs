...
use xcm_executor::traits::{
	...
	ExportXcm, FeeManager, FeeReason, OnResponse, ProcessTransaction, Properties, ShouldExecute, TransactAsset,
...
impl<Config: config::Config> xcm_executor::traits::XcmAssetTransfers for XcmExecutor<Config> {
		...
		match instr {
			...
			DepositReserveAsset { assets, dest, xcm } => {
				...
				let result = Config::TransactionalProcessor::process(|| {
					...
					let mut message_to_weigh = vec![ReserveAssetDeposited(to_weigh.into()), ClearOrigin];
