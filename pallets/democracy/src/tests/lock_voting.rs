...
fn lock_voting_should_work() {
	new_test_ext().execute_with(|| {
		...
		for i in 1..=5 {
			assert_eq!(balance_frozen_of(i), i * 10);
		}
		...
		assert_eq!(balance_frozen_of(1), 0);
		assert_eq!(balance_frozen_of(2), 20);
		assert_eq!(balance_frozen_of(3), 30);
		assert_eq!(balance_frozen_of(4), 40);
		assert_eq!(balance_frozen_of(5), 0);
		...
		fast_forward_to(26);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(1), 2));
		assert_eq!(balance_frozen_of(2), 0);
	});
}
...
fn multi_consolidation_of_lockvotes_should_be_conservative() {
	new_test_ext().execute_with(|| {
		...
		fast_forward_to(8);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert!(balance_frozen_of(5) >= 20);

		fast_forward_to(14);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert!(balance_frozen_of(5) >= 10);
