...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// # <weight>
		/// Weight: O(1)
		/// - Reads: InflationConfig, MaxCollatorCandidateStake, MaxSelectedCandidates
		/// # </weight>
		...
		T: Config + pallet_authorship::Config + pallet_session::Config,
	{
		...
		/// - Reads: CandidatePool, TotalCollatorStake, Balance, InflationConfig,
		...
		fn note_author(author: T::AccountId) {
			...
			if let Some(state) = CandidatePool::<T>::get(author.clone()) {
				...
				let TotalStake { collators: total_collators, delegators: total_delegators } =
				...
				let authors_per_round =
					<BalanceOf<T>>::from(authors.len().saturated_into::<u128>());

				// Reward collator
				let amt_due_collator = inflation_config.collator.compute_reward::<T>(
					state.stake,
					c_staking_rate,
					authors_per_round,
				);
