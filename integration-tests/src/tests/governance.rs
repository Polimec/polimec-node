...
use frame_support::traits::{OnInitialize, WithdrawReasons};
...
use polimec_base_runtime::{Balances, RuntimeOrigin, Vesting, Council, TechnicalCommittee, GrowthTreasury, Democracy, Preimage};
use tests::defaults::*;
...
fn democracy_works() {
	...
	PolimecBase::execute_with(|| {
		...
		assert_eq!(Balances::balance_frozen(&polimec_base_runtime::RuntimeFreezeReason::Democracy(pallet_democracy::FreezeReason::Vote), &alice), 0);
		do_vote(alice.clone(), 0, true, 100 * PLMC);
		assert_eq!(Balances::balance_frozen(&polimec_base_runtime::RuntimeFreezeReason::Democracy(pallet_democracy::FreezeReason::Vote), &alice), 100 * PLMC);
	});
...
fn treasury_proposal_accepted_by_council() {
	...
	PolimecBase::execute_with(|| {
		...
		let proposal = polimec_base_runtime::RuntimeCall::GrowthTreasury(
			pallet_treasury::Call::approve_proposal { proposal_id: 0 }
		);
...
fn run_gov_n_blocks(n: usize)
{
	for _ in 0..n {
		PolimecBase::execute_with(|| {
			...
			polimec_base_runtime::System::initialize(
				&next_block_number,
				&header.hash(),
				&pre_digest,
			);
