...
fn migration_is_sent() {
	...
	let project_id = Polimec::execute_with(|| {
		inst.create_finished_project(
			...
			IntegrationInstantiator::generate_bids_from_total_usd(
				Perquintill::from_percent(40) * (sp_runtime::FixedU128::from_float(1.0).checked_mul_int(100_000 * ASSET_UNIT).unwrap()),
			...
			IntegrationInstantiator::generate_contributions_from_total_usd(
				Perquintill::from_percent(50) *	(sp_runtime::FixedU128::from_float(1.0).checked_mul_int(100_000 * ASSET_UNIT).unwrap()),
				sp_runtime::FixedU128::from_float(1.0),
				default_weights(),
	...
	Polimec::execute_with(|| {
		...
		let evaluations = pallet_funding::Evaluations::<PolimecRuntime>::iter_prefix_values((project_id,)).collect::<Vec<_>>();
		dbg!(evaluations);
	});
	...
	Polimec::execute_with(|| {
		...
		println!("Polimec events:");
		dbg!(Polimec::events());
		let mut user_evaluations = pallet_funding::Evaluations::<PolimecRuntime>::iter_prefix_values((project_id, eval_1()));
		let mut user_bids = pallet_funding::Bids::<PolimecRuntime>::iter_prefix_values((project_id, eval_1()));
		dbg!(user_bids.clone();
		let mut user_contributions = pallet_funding::Contributions::<PolimecRuntime>::iter_prefix_values((project_id, eval_1()));
