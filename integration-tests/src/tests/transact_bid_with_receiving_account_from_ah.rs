use crate::{
	...
	PolimecParachainSystem, PolimecRuntime, PolkadotRelay, PolkadotSystem, ALICE,
};
...
fn transact_bid_with_receiving_account_from_ah() {
	...
	AssetHubWestendNet::execute_with(|| {
		...
		let current_block_number = AssetHubSystem::block_number();
		AssetHubSystem::set_block_number(current_block_number + 1);
		println!("Block number set to {}", current_block_number + 1);

		let relay_block_number = PolkadotSystem::current_block_number();
		println!("Relay block number set to {}", relay_block_number);
		let current_block_number = RelaychainDataProvider::<AssetHubRuntime>::current_block_number();
		PolkadotSystem::set_block_number(current_block_number - 1);
		println!("Block number set to {}", current_block_number - 1);
	});
