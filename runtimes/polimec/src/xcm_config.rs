...
impl frame_support::traits::Contains<Location> for SupportedAssets {
	fn contains(l: &Location) -> bool {
		match l.unpack() {
			(1, []) |
			(1, [Parachain(1000), PalletInstance(50), GeneralIndex(1337)]) |
			(1, [Parachain(1000), PalletInstance(50), GeneralIndex(1984)]) => true,
			_ => false,
		}
	}
}
...
impl MaybeEquivalence<Location, AssetIdPalletAssets> for SupportedAssets {
	fn convert(asset: &Location) -> Option<AssetIdPalletAssets> {
		match asset.unpack() {
			(1, []) => Some(10),
			(1, [Parachain(1000), PalletInstance(50), GeneralIndex(1337)]) => Some(1337),
			(1, [Parachain(1000), PalletInstance(50), GeneralIndex(1984)]) => Some(1984),
			_ => None,
		}
	}
